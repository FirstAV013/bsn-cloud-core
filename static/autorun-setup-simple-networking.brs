...
Sub Main()
  ...
  RunSetup(setupVersion$, diagnostics, diagnosticCodes)
...
Sub RunSetup(setupVersion$ as string, diagnostics as object, diagnosticCodes as object)
  ...
  while true
    ...
    if (type(msg) = "roUrlEvent") then
      ...
      if type(m.networking.checkAlarm) = "roTimer" and stri(msg.GetSourceIdentity()) = stri(m.networking.checkAlarm.GetIdentity()) then
        ...
        ' If downloads has finished, move forward. If not, set the proceedAfterRegistrationProcessComplete flag to true
        if m.networking.contentDownloadComplete then
          ...
          m.networking.proceedAfterRegistrationProcessComplete = true
        end if
      end if
      ...
      if msg.GetSourcePort() = m.networking.wsUdpSocketPort% then
...
Function InitializeNetworkDownloads(setupParams as object) as boolean
  ...
  videoMode = CreateObject("roVideoMode")
    ...
    m.deviceSetupSplashScreen = SetDeviceSplashScreenMessageByType(invalid, "sfn_initialize", m.msgPort)
  end if
  ...
  if type(supervisorRegistrySection) <> "roRegistrySection" then
    m.diagnostics.PrintDebug("Error: Unable to create supervisorRegistrySection roRegistrySection"): stop
  end if
  ...
  if registrationFlag = "yes" then
    m.proceedAfterRegistrationProcessComplete = true
  end if

  m.contentDownloadComplete = false

  ' This timer will be triggered in function PoolEvent
  m.registrationResponseTimer = CreateObject("roTimer")
  m.registrationResponseTimer.SetPort(m.msgPort)
  m.registrationResponseTimer.SetElapsed(60, 0)
  ...
  ' Will put a RestartApplication() at the end of setup if restartRequired = true.
  if (setupParams.restartRequired = true) then m.restartRequired = true
    ...
    if msg.GetInt() = m.URL_EVENT_COMPLETE then
      ...
      if msg.GetResponseCode() = 200 then
        ...
        if type(videoMode) = "roVideoMode" then
          ...
          if autoscheduleFilePath = "" then
            m.diagnostics.PrintDebug("### Set the setup splash screen for SFN")
            m.deviceSetupSplashScreen = SetDeviceSplashScreenMessageByType(m.deviceSetupSplashScreen, "sfn_complete", m.msgPort)
          end if
        end if
        ...
        m.diagnostics.PrintDebug("### Failed to download sync list: " + stri(msg.GetResponseCode()))
        m.SendError("Failed to download sync list", "", str(msg.GetResponseCode()), "")
        RestartIfNecessary(m, false)
        ' only set splash screen if device supports video
        videoMode = CreateObject("roVideoMode")
        if type(videoMode) = "roVideoMode" then
          m.deviceSetupSplashScreen = SetDeviceSplashScreenMessageByType(m.deviceSetupSplashScreen, "sfn_error", m.msgPort)
        end if
      end if
    else
  ...
  if msg.GetSourceIdentity() = m.syncPool.GetIdentity() then
    if (msg.GetEvent() = m.POOL_EVENT_FILE_DOWNLOADED) then
      ...
      if event.GetEvent() <> m.EVENT_REALIZE_SUCCESS then
        ...
        ' Entering here means finished downloading files and ready to restart.
        ' If proceedAfterRegistrationProcessComplete flag is set to true, then device has registered. -> restart to move forward.
        ...
        ' If flag is false, then set contentDownloadComplete flag and wait for bootstrap udp message for a min -> Eventloop will both wait for a timer and a udp message.
        m.contentDownloadComplete = true
        m.registrationResponseTimer.Start()
        return
      end if
...
Function newLogging() as object
  ...
  logging.OpenLogFile = OpenLogFile
  logging.CloseLogFile = CloseLogFile
  logging.WriteToLog = WriteToLog
  logging.RotateLogFiles = RotateLogFiles
