...
async function uploadLocalFile(
    fullPath: string,
    bsnVirtualPath: string
): Promise<BsUploadJobResult> {
    ...
    const uploadJob = cmCreateBsnUploadJob(
        "TestJob",
        uploadJobSpecs,
        null,
        progressHandler
    );
    await bsnGetSession().activate(
        credentials.user,
...
async function doUploadLocalFile() {
    try {
        ...
        if (fileResult.error) {
            ...
            console.log(
                "Upload time:",
                (new Date().getTime() - uploadResult.startTime.getTime()) /
                    1000,
                "seconds"
            );
